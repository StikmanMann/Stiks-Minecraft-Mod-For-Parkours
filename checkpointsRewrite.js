...
export {oldParkour}
class Checkpoint extends BookData{

    #spawnOffset = {x:0.5, y:0, z:0.5}
    /**
     * @type {Player[]}
     */
    #players;
        ...
        for(let i = 0; i < this.#checkpoints.length; i++){
            ...
            for(const player of this.#players){
                ...
                if(player.location.y < -59.5){
                    ...
                    let failTp = VectorFunctions.addVector(this.#checkpoints[ScoreboardFunctions.getScore(player, "currentCp")], this.#spawnOffset)
                    player.teleport({x: player.location.x, y:player.location.y + 1.5, z: player.location.z})
                    await DrawFunctions.drawGraph(20, player.location, VectorFunctions.addVector(this.#checkpoints[ScoreboardFunctions.getScore(player, "currentCp")], this.#spawnOffset), player)
                ...
                let boxSize = 1
                if(CollisionFunctions.insideBox(player.location, VectorFunctions.subtractVector(cp, {x:boxSize, y:0.1, z:boxSize}), VectorFunctions.addVector(cp, {x:boxSize, y:2, z:boxSize}), false)){
     ...
     */
    tpToCp(player, index){
        player.teleport(VectorFunctions.addVector(this.#checkpoints[index], this.#spawnOffset))
...
let oldParkour = new Checkpoint({x:0, y:-60, z:0}, 0)
